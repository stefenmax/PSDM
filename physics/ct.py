...
class CT():
    def __init__(self, img_width, radon_view, uniform=True, circle=False, device='cuda:0'):
        if uniform:
            ...
            theta = torch.arange(radon_view)*180/radon_view
            theta_all = torch.arange(radon_view)*180/radon_view
...
class CT_LA():
    ...
    Limited Angle tomography
    """
    def __init__(self, img_width, radon_view, angle_range, uniform=True, circle=False, device='cuda:0'):
        # angle_range is a tuple indicating the start and end angles
        if uniform:
            theta = np.linspace(angle_range[0], angle_range[1], radon_view, endpoint=False)
        else:
            theta = torch.linspace(angle_range[0], angle_range[1], radon_view)
