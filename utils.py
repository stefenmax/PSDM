...
def restore_checkpoint(ckpt_dir, state, device, skip_sigma=False, skip_optimizer=False):
    ckpt_dir = Path(ckpt_dir)
#    breakpoint()
#    ckpt_dir = '/mnt/new_ssd/shuo/code/DDPM_related/DiffusionMBIR-main/workdir/AAPM256_norm_new/checkpoints/checkpoint_1.pth'
#    ckpt_dir = '/mnt/new_ssd/shuo/code/DDPM_related/DiffusionMBIR-main/workdir/AAPM256/checkpoints-selected/checkpoint_185.pth'
#    ckpt_dir = '/mnt/new_ssd/shuo/code/DDPM_related/DiffusionMBIR-main/workdir/AAPM256_norm/checkpoints/checkpoint_285.pth'
    ckpt_dir = Path(ckpt_dir) # 185 224 748 330_A100 225_new
#    breakpoint()
    if not ckpt_dir.suffix == '.pth':
        logging.warning(f"No checkpoint found at {ckpt_dir}. "
                        f"Returned the same state as input")
        return state
    else:
